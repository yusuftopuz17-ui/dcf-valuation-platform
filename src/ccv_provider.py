...
def company_record(ticker: str, years: int = 5, period: str = "TTM") -> dict[str, Any]:
    ...
    record = {
        ...
        "Equity": float(latest["Equity"]) if np.isfinite(latest["Equity"]) else np.nan,
        ...
        "Net Debt": market["Net Debt"],
        "P/S": market["Market Cap"] / revenue if revenue > 0 else np.nan,
        "P/B": market["Market Cap"] / latest["Equity"] if np.isfinite(latest["Equity"]) and latest["Equity"] > 0 else np.nan,
        "Forward P/E": pd.to_numeric(info.get("forwardPE"), errors="coerce"),
        "PEG": pd.to_numeric(info.get("trailingPegRatio") or info.get("pegRatio"), errors="coerce"),
        "EPS Growth": pd.to_numeric(info.get("earningsGrowth"), errors="coerce"),
